...
🚧 **Early development.** `0.3.0` is the latest tagged release — Phase 1 complete. Marshal speaks in its own voice through the `marshal` subcommand namespace (`config`, `what-now`, `help`), emits modernization tips for deprecated Git forms, appends actionable hints below git's own stderr when common commands fail (13 rules), recommends a concrete next step on demand via `marshal what-now`, and ships an on-CLI reference via `marshal help` (context-aware, five topics). Every namespace command supports `--json` for scripting. Any command Marshal does not intercept still passes through to `git` byte-for-byte. Workspace features (the original thesis) arrive next — see [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
- `0.3.0` — 2026-04-27. Phase 1 complete. 13 actionable error hints (with `errors.actionable_hints` opt-out), `marshal what-now` with a 9-rule priority chain, the Command/Renderable substrate locking Invariant 10 (Open/Closed via Strategy), global `--json` for every marshal-namespace command, and a context-aware `marshal help` with five reference topics. Tagged as [`v0.3.0`](https://github.com/matutetandil/marshal/releases/tag/v0.3.0). Install from source: `cargo install --git https://github.com/matutetandil/marshal --tag v0.3.0`.
