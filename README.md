...
| 3 — Workspace modifications   | ⏳ in progress                  | The three zones (declared / staged / working). `ws add`, `ws unstage`, `ws restore (<repo> | --all)`, `ws reset`, `ws commit`, `ws branch` (thin), `ws switch` shipped; pre-flight Strategy + parallel framework consolidated; granular-scope `ws branch` (F.5) next. |
